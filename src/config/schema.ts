...
export const androidCliConfigSchema = Joi.object<AndroidCliConfig>({
  ...
  logcat: Joi.object({
    ...
    template: Joi.string().default('sh -c "adb logcat -v color --pid=$(adb shell pidof -s {{package_name}})"').description('Logcat command template with variable substitution'),
  }).default({ clearOnStart: true, colorize: true, template: 'sh -c "adb logcat -v color --pid=$(adb shell pidof -s {{package_name}})"' }),
...
export const defaultConfig: AndroidCliConfig = {
  ...
  logcat: {
    clearOnStart: true,
    colorize: true,
    template: 'sh -c "adb logcat -v color --pid=$(adb shell pidof -s {{package_name}})"',
  },
  adbReverse: {
    enabled: false,
    ports: [8081],
