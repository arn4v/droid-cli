...
export const androidCliConfigSchema = Joi.object<AndroidCliConfig>({
  ...
  logcat: Joi.object({
    ...
    template: Joi.string().default('adb -s {{device_id}} logcat -v color --pid=$(adb -s {{device_id}} shell pidof -s {{package_name}})').description('Logcat command template with variable substitution'),
  }).default({ clearOnStart: true, colorize: true, template: 'adb -s {{device_id}} logcat -v color --pid=$(adb -s {{device_id}} shell pidof -s {{package_name}})' }),
